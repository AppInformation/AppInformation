...
function deleteProductCategory(id,type) {
 var bool=window.confirm("确认删除此分类信息么?");
	if(bool){
		$.ajax({
	        url: contextPath + "/ProductCategoryServlet",
	        method: "post",
	        data: {
	            id: id,
	            type:type,
...
function deleteById(id) {
	...
	if(bool){
		$.ajax({
	        url: contextPath + "/ProductListServlet",
	        method: "post",
	        data: {
	            id: id,
...
function addUser() {
    ...
    $.ajax({
        ...
        success: function (jsonStr) {
            var result = eval("(" + jsonStr + ")");
            if (result == 1) {
                window.location.href=contextPath+"cn.easubuy/web/backend/userDeleteServlet?action=queryUserList";
            }else{
            	showMessage(result.message);
            }
        }
